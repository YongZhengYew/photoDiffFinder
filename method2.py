...
def openBoundingBoxes(boundingBoxArray, bpp=None):
	...
	for boundingBox in boundingBoxArray:
		if boundingBox.width > 0 and boundingBox.height > 0:
			...
			if SAVEFLAG:
				print("Saving from Method 2...")
				newImage.save("{absPath}/METHOD2/METHOD2_{number}_{x}x{y}.png".format(absPath=ABSPATH, number=count, x=boundingBox.x, y=boundingBox.y))
